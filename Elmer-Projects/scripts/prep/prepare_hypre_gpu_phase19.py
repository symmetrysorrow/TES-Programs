...
PHASE20_PROBE_ITERATIONS = 15


def phase20_probe_name(variant: str, use_gpu: bool) -> str:
    return f"case_p20_hypre_block_{variant}_{'gpu' if use_gpu else 'cpu'}_probe"
...
def bounded_block_case(source: dict, name: str, use_gpu: bool, full: bool) -> dict:
    """Create a short, non-accepting convergence probe.

    The probe has the same mesh/restart/first timestep as the Phase19
    candidate, but bounds the outer Krylov work and keeps the final iterate
    available for diagnostics.  Lower and full are deliberately generated by
    the same path so neither candidate is downstream-gated on the other.
    """
    result = block_diag_case(source, name, use_gpu)
    result["comparison_time_grid"] = {
        "mode": "Phase20 bounded convergence probe",
        "purpose": "peer comparison of lower/full block candidates before production timing",
        "reference_case": SOURCE_CASE,
        "same_mesh_restart_timestep": True,
        "outer_max_iterations": PHASE20_PROBE_ITERATIONS,
        "linear_solver": f"native block {'full' if full else 'lower'} + HYPRE BoomerAMG",
    }
    result["timesteps"] = truncate(result["timesteps"], 1)
    result["output_intervals"] = [1]
    result["series_file"] = f"{name}_series.csv"
    result["iteration_series_file"] = f"{name}_iterations.csv"
    result["output_file_path"] = f"../work/meshes/{result['mesh']}/{name}.result"
    result["solver"] = dict(result["solver"])
    result["solver"].update(
        {
            "linear_system": f"iterative_hypre_block_{'full' if full else 'lower'}"
            + ("_gpu" if use_gpu else ""),
            "linear_system_max_iterations": PHASE20_PROBE_ITERATIONS,
            "linear_system_abort_not_converged": False,
            "linear_system_residual_output": 1,
            "block_nested_primal_max_iterations": 1,
            "block_schur_inner_tolerance": 1.0e-4,
            "block_schur_max_iterations": 30,
            "block_schur_restart": 30,
            "block_schur_probe": True,
            "block_schur_probe_prefix": name,
            "matrix_dump_prefix": name,
            "nonlinear_max_iterations": 1,
            "nonlinear_convergence_tolerance": 1.0e-3,
        }
    )
    result["max_output_level"] = 10
    return result
...
def main() -> None:
    ...
    # Phase20 bounded probes.  These are peer candidates: always create all
    # four combinations, even if the Phase19 lower production attempt is
    # incomplete.  GPU probes are harmless to generate on CPU-only hosts and
    # are skipped by the runtime launcher when no GPU build/device is present.
    for variant, full in (("lower", False), ("full", True)):
        for use_gpu in (False, True):
            name = phase20_probe_name(variant, use_gpu)
            project["cases"][name] = bounded_block_case(
                project["cases"][SOURCE_CASE], name, use_gpu, full
            )
