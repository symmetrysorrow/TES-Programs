...
def make_case(base: dict, name: str, variant: str, gpu: bool,
              tol: float, max_iters: int, cycles: int) -> dict:
    ...
    case["timesteps"] = case["timesteps"][:1]
...
def build(input_path: Path, output_path: Path, all_points: bool = False) -> dict:
    ...
    for variant, gpu, point in itertools.product(("lower", "full"), (False, True), selected_points(all_points)):
        ...
        cases[name] = make_case(source, name, variant, gpu, tol, max_iters, cycles)
    project["cases"] = cases
...
def main() -> int:
    ...
    project = build(args.input, args.output, args.all)
