...
DEFAULT_EP_PATH = Path("legacy/meshes/mesh_fragment_centered_test/case_constant_power_fragment_test.ep")
DEFAULT_BODY_ID = 101


def main() -> None:
    ep_path = Path(sys.argv[1]) if len(sys.argv) > 1 else DEFAULT_EP_PATH
    mesh_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else ep_path.parent
    ...
    mesh_elements_path = mesh_dir / "mesh.elements"
    if not mesh_elements_path.exists():
        raise RuntimeError(f"mesh.elements not found next to EP output: {mesh_elements_path}")

    tes_nodes: set[int] = set()
    for line in mesh_elements_path.read_text(encoding="utf-8", errors="replace").splitlines():
        parts = line.split()
        if not parts:
            continue
        body = int(parts[1])
        if body == DEFAULT_BODY_ID:
            tes_nodes.update(int(node_id) - 1 for node_id in parts[3:])
