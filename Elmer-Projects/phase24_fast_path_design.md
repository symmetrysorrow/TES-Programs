...
The native audit isolated the adaptive HYPRE plateau to stale static-matrix reuse across the two unequal-history half steps. The Phase24 cache key now includes the active BDF order and leading mass coefficient `a0` in addition to timestep, lumping mode, and CSR size. Fast static/prism assembly uses the same unequal-step BDF2 coefficients as the generic `VBDFLocal`/`VBDF_CRS` paths, and adaptive debug logging publishes `h_new`, `h_old`, `r`, `a0/a1/a2`, and cache decisions. In the two-step CPU diagnostic, BDF2 half-1 used `(1.333333e6,-1.5e6,1.666667e5)` and half-2 used `(1.5e6,-2.0e6,5.0e5)`; the changed `a0` caused a fresh static build. With linear tolerance `5e-7` and the production temporal tolerance `0.002`, that run reached `accepted=2, rejected=0, bdf2=1, ALL DONE`; the linear tolerance was not relaxed. The strict debug temporal tolerance `1e-4` still over-resolves the estimator relative to the observed linear-solver noise and was not claimed complete. A reuse-off comparison was started but stopped after roughly eleven minutes without completion because every nonlinear solve rebuilt the HYPRE lifecycle. The production 31-us adaptive campaign and full physical-parity comparison remain outstanding.

The generator now validates adaptive start/end against staged timestep duration and restart time, clips physical events outside the integration interval, and the Python output counter supports excluding the shared start of an adjacent span so requested endpoints cannot be double-counted. The full Python suite passes (`118 passed`).
