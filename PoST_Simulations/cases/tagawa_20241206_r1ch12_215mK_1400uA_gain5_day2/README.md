...
Known acquisition facts are sufficient to fix the FFT grid: `rate=500000 Hz`, `samples=100000`, `cutoff=10000 Hz`, and `df=5 Hz`. The target experimental post-analysis ASD is summarized at the requested frequencies in [`comparison_summary.json`](comparison_summary.json); the current readiness decision is in [`audit_status.json`](audit_status.json). The baseline physical noise sources remain enabled by PoST semantics. Only empirical white/readout floors, residual-derived terms, Lorentzian resistance fluctuation, and hanging TES are disabled. The simulation columns are intentionally `null`, because an absolute no-added-noise comparison would otherwise manufacture a physical operating point.
...
Readiness is capability-specific in [`audit_status.json`](audit_status.json), and the dependency/identifiability maps are [`parameter_dependency.json`](parameter_dependency.json) and [`parameter_identifiability.json`](parameter_identifiability.json). To open the next gate, add an independently sourced target calibration/operating-point record covering at least `T_c`, `R`, `G_tes-bath`, and the target-linked bath value; then add the remaining stability/thermal parameters. Copy only accepted values into `input.json`, rerun the operating-point/stability and Python/C++ parity audits, and only then populate the simulation ASD and ratio columns.
