"""Contract tests for the corrected frozen high-frequency comparison."""
...
from PoST_Simulations.subScript.high_frequency_noise_comparison import (  # noqa: E402
    intervals,
    log_interp,
)
...
def test_log_interp_is_log_log():
    ...
    np.testing.assert_allclose(
        log_interp(x, y, np.array([3.16227766])),
        [10.0],
        rtol=1e-7,
    )
...
def test_primary_band_excludes_low_frequency_from_score():
    ...
    assert '"low_frequency_excluded_from_selection": True' in text


def test_measurement_chain_is_explicit_and_time_domain():
    source = (
        Path(__file__).parents[1]
        / "subScript"
        / "high_frequency_noise_comparison.py"
    ...
    assert "analysis_bessel_cutoff_Hz" in text
    assert "finite_record_post_analysis_asd" in text
    assert '"simulation_time_domain_realization": True' in text
    assert "100 kHz analog hardware Bessel" in text
    assert "10 kHz digital Bessel filtfilt" in text
...
def test_display_band_covers_full_positive_frequency_range():
    ...
    assert "plot_freq = np.logspace" in text
