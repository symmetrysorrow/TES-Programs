...
def test_readiness_is_split_by_capability_and_physical_sources_stay_enabled():
    ...
    for capability in capabilities.values():
        assert "conditionally_ready" in capability
        assert "inadmissible_parameters" in capability
        assert "provenance_status" in capability
    assert "T_c" in capabilities["operating_point_ready"]["missing_parameters"]
    assert capabilities["operating_point_ready"]["provenance_status"]["T_bath"] == "conditional"
    assert "readout_gain_transfer" in capabilities["absolute_comparison_ready"]["missing_parameters"]
...
def test_report_names_post_analysis_and_does_not_back_calculate_pre_analysis():
    ...
    assert "experimental_pre_analysis_asd" in report["spectrum_semantics"]
    ...
    for row in report["points"]:
        ...
        assert "experimental_pre_analysis_normalized" in row
