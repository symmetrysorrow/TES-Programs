...
def test_cpp_hanging_tes_left_right_symmetry(tmp_path):
    ...
    expected_thermal = -(
        (1.0 - cpp["loop_gain"]) * parameters["G_tes-bath"] / parameters["C_tes"]
        + parameters["G_abs-tes"] / parameters["C_tes"]
        + parameters["G_tes-hanging"] / parameters["C_tes"]
    )
    assert np.isclose(cpp["tes1_time_block"][3], expected_thermal, rtol=2e-12)
    np.testing.assert_allclose(cpp["tes1_hanging_row"], cpp["tes2_hanging_row"], rtol=2e-12, atol=1e-7)
