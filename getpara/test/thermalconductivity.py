...
T_bath_list = [160,170,180,190,200]					#使用する熱浴温度のリスト[mK]
...
T_c_f = 160				#nについても同様。
n_s = 4.0					#注意として、全てのパラメタで下限は0.0である。
n_f = 4.0		#またT_cについては、相転移温度±5.0[mK]くらいが良い。
...
x = np.arange(np.array(T_bath_list)[0]-2.0,np.array(T_bath_list)[T_n-1]+2.0,0.00001)
f = (a*(b*1.0e-3)*(1.0/c))*(1.0-(np.array(x)*1.0e-3/(b*1.0e-3))**c)
...
folder = os.path.exists('./output_n=4(50%)')
if not folder:
	os.mkdir('output_n=4(50%)')
os.chdir('./output_n=4(50%)')
...
plt.plot(x,f*1.0e+9,color ='black',label = 'Fitting',linestyle ='-',linewidth = 2)
